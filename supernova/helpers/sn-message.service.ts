...
// This is a service that receives iframe messages from the parent.
...
const processSetCssVars = (cssVars: Record<string, string>) => {
  ...
  Object.entries(cssVars).forEach(([varName, value]) => {
    ...
    if (
      (varName === "--font-sans" || varName === "--font-serif") &&
      typeof value === "string"
    ) {
  ...
  Object.values(fontVars).forEach((fontValue) => {
    ...
    if (
      document.head.querySelector(`link[data-theme-font="${formattedFont}"]`)
    ) {
      return;
    }
...
export class SnMessageService implements OnDestroy {
  private boundReceive: (event: MessageEvent) => void;

  constructor() {
    this.boundReceive = this.receive.bind(this);
    if (typeof window !== "undefined") {
      window.addEventListener("message", this.boundReceive);
    }
  }

  ngOnDestroy(): void {
    if (typeof window !== "undefined") {
      window.removeEventListener("message", this.boundReceive);
    }
  }
